<script lang="ts">
	...
	const getNumericScore = (score: AnimeBase['score']) => {
		const parsedScore = score === null ? Number.NEGATIVE_INFINITY : Number(score);
		return Number.isFinite(parsedScore) ? parsedScore : Number.NEGATIVE_INFINITY;
	};
...
<div class="max-w-2xl mx-auto">
		...
		<div class="relative">
			...
			{#if isSearching}
				<div class="absolute right-4 top-1/2 -translate-y-1/2">
					<div class="inline-block h-5 w-5 animate-spin rounded-full border-2 border-solid border-indigo-600 border-r-transparent"></div>
				</div>
			{/if}
		</div>
				...
				>
					<div class="flex gap-4">
						<div class="flex-1">
							...
							{#if anime.genres.length > 0}
								<div class="flex flex-wrap gap-1.5 mb-2">
									{#each anime.genres.slice(0, 4) as genre}
										<span class="px-2 py-0.5 text-xs rounded-full bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300">
											{genre}
										</span>
									{/each}
								</div>
							...
							<div class="flex items-center gap-3 mt-2 text-xs text-slate-500 dark:text-slate-500">
								{#if anime.score}
	...
	{:else if !isSearching && trimmedQuery.length >= 2}
		<div class="text-center py-12 text-slate-500 dark:text-slate-400">
			...
			<p class="text-sm mt-1">Try a different title or check your spelling</p>
		</div>
	{/if}
</div>
