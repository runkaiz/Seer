...
class AnimeHistoryItem(AnimeBase):
    ...
    has_seen: bool = Field(default=True, description="Whether the user has seen this anime")
    rating: Optional[UserRatingEnum] = Field(None, description="User's rating (POSITIVE/NEUTRAL/NEGATIVE)")


class AnimeRecommendation(AnimeBase):
    """Anime recommendation with explanation."""
    recommendation_reason: str = Field(..., description="Why this anime was recommended - focusing on discovery and expanding horizons")
